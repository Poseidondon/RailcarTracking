...
def load_config(cfg_type: str, cfg_path='default.yaml'):
    all_configs = {'tracker': ['device', 'save', 'project', 'name', 'exist_ok', 'conf', 'iou', 'max_det', 'tracker',
                   ...
                   'clipper': ['source', 'period', 'repeats', 'verbose', 'timeout_factor', 'ext', 'out_dir',
    ...
    if cfg_type not in all_configs:
        raise ValueError(f"Config '{cfg_type}' does not exist. Available configs are {', '.join(all_configs)}.")
    else:
        # load proper args
